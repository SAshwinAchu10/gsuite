...
function scheduleShifts() {
  ...
  for (i in data) {
    ...
    if (insert) {
      ...
      var newEvent = CalendarApp.getCalendarById(calId).createEvent(title != null ? title : 'Interview', startTime, endTime, { guests: `${interviewer},${candidate}` });
      row[5] = newEvent.getId();
      changes++;
    }
  }
  if (changes) {
